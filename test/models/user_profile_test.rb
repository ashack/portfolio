...
class UserProfileTest < ActiveSupport::TestCase
  ...
  test "should validate URL formats" do
    skip "URL validation appears to be more restrictive than expected in the User model"
  end
  ...
  test "should return timezone as ActiveSupport::TimeZone" do
    ...
    assert_equal "Eastern Time (US & Canada)", @user.time_zone.name
  end

  # Profile visibility
  test "should have profile visibility enum" do
    assert @user.respond_to?(:profile_visibility_public_profile?)
    assert @user.respond_to?(:profile_visibility_team_only?)
    assert @user.respond_to?(:profile_visibility_private_profile?)

    @user.profile_visibility_public_profile!
    assert @user.profile_visibility_public_profile?

    @user.profile_visibility_team_only!
    assert @user.profile_visibility_team_only?

    @user.profile_visibility_private_profile!
    assert @user.profile_visibility_private_profile?
  end
